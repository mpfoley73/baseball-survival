...
cox_dat %>% 
  ...
  filter(debut_age > 30) %>%
  # filter(playerID == "acostme01") %>%
  select(playerID, birthDate, debut, tstart, tstop, death, decade, age, fu_time, fu_status) 
cox_dat %>% janitor::tabyl(age, death) %>% janitor::adorn_percentages() %>% janitor::adorn_ns()
cox_dat %>% 
  group_by(age) %>%
  summarize(n = n(), surv = mean(death), age_len = mean(tstop - tstart))

new_dat <- data.frame(age = c("[0,25)", "[30,35)", "[50,55)")) %>% mutate(strata = as.factor(row_number()))
...
gt_2_cox_2022 <- tbl_regression(mdl_2_cox_2022, exponentiate = FALSE) %>% 
  add_significance_stars(hide_se = TRUE)
gt_3_cox_2022 <- tbl_regression(mdl_3_cox_2022, exponentiate = FALSE) %>% 
  add_significance_stars(hide_se = TRUE)
gt_4_cox_2022 <- tbl_regression(mdl_4_cox_2022, exponentiate = FALSE) %>% 
  add_significance_stars(hide_se = TRUE)
gt_5_cox_2022 <- tbl_regression(mdl_5_cox_2022, exponentiate = FALSE) %>% 
  add_significance_stars(hide_se = TRUE)
gt_6_cox_2022 <- tbl_regression(mdl_6_cox_2022, exponentiate = FALSE) %>% 
  add_significance_stars(hide_se = TRUE)
gt_9_cox_2022 <- tbl_regression(mdl_9_cox_2022, exponentiate = FALSE) %>% 
  add_significance_stars(hide_se = TRUE)

gtsummary::tbl_merge(
 list(gt_1_cox_2022, gt_2_cox_2022, gt_3_cox_2022, gt_4_cox_2022, gt_5_cox_2022, 
      gt_6_cox_2022, gt_9_cox_2022), 
 tab_spanner = c("Model 1", "Model 2", "Model 3", "Model 4", "Model 5", 
                 "Model 6", "Model 9")
) 
```

Model 1 shows that the odds of death increase with age. Players aged 40–44 are `r gtsummary::inline_text(gt_1_cox_2022, age, "[40,45)")` times as likely to die over the follow-up period as players aged 20–24 (Authors report exp(1.32) = 3.74), those aged 50–54 are `r gtsummary::inline_text(gt_1_cox_2022, age, "[50,55)")` times as likely to die (Authors report exp(2.39) = 10.9), and players aged 70–74 are `r gtsummary::inline_text(gt_1_cox_2022, age, "[70,75)")` times as likely to die (Authors report exp(4.00) = 54.6). The **Lahman**-based data produced higher mortality risk estimates at all age groups for Model 1.
