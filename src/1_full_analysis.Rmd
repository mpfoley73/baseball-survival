...
bio_country %>% arrange(desc(n)) %>% head(10)

world_dat %>%
  left_join(bio_country, by = c("name" = "birth_country")) %>%
  replace_na(list(n = 0)) %>%
  mutate(n = if_else(name == "United States", 0, n)) %>%
  ggplot() +
  geom_sf(aes(fill = n)) +
  geom_text(data = world_centroid %>% 
              filter(name %in% (c("Mexico", "Dominican Rep.", "Cuba", "Venezuela"))), 
  ...
  coord_sf(xlim = c(-112.15, -54.12), ylim = c(-7.65, 33.97), expand = TRUE) +
  theme_light() +
  labs(title = "Where players come from.") 

length(unique(world_dat$NAME))
