...
Today, 28% of MLB players were born outside the US. The Domincan Republic and Venezuela are the largest sources of international talent.
...
# Modify country names in bio to match those in world_dat. Then summarize data
# for plot.
bio_country <- bio %>% 
  mutate(
...
bio_state_n <- bio_state %>%
  filter(career_status == 0 & birth_country == "USA") %>%
  count(birth_country, birth_state) %>%
  mutate(pct = n / sum(n))
...
world_dat %>%
  ...
  mutate(
    lbl = if_else(name %in% c("United States", "Mexico", "Dominican Rep.", "Cuba", "Venezuela"),
                  glue("{abbrev}\n{percent(pct, 1)}"),
                  glue(""))
  ) %>%
  ggplot() +
  geom_sf(aes(fill = pct), show.legend = FALSE) +
  ...
  scale_fill_gradient(low = "gray99", high = "slategray") +
  ...
  mlb_theme +
  labs(
    title = "Fig 5a. Top International Sources of Talent.", 
    subtitle = "Historical count of major leaguers.",
...
state_dat %>%
  ...
  left_join(bio_state_n_2, by = c("name" = "birth_state")) %>%
