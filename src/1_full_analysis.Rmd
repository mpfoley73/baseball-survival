...
p4 + 
  ggtitle(paste("Fig. 4", title_text))
```
...
```{r}
title_text <- "MLB became more international after WWII."
p7 <- bio_country %>%
...
p7 + ggtitle(paste("Fig. 7", title_text))
```
...
```{r}
(km_fit <- survfit(Surv(career_yrs, career_status) ~ hof, data = bio))
```
The median career length is `r summary(km_fit)$table[1, "median"] %>% comma(.1)` years for non-Hall of Famers, `r summary(km_fit)$table[2, "median"] %>% comma(.1)` years for those in the Hall. 
...
```{r}
(km_fit <- survfit(Surv(play_lastgame_life_yrs, career_status) ~ hof, data = bio))
```
The median retirement age is `r summary(km_fit)$table[1, "median"] %>% comma(.1)` years for non-Hall of Famers, `r summary(km_fit)$table[2, "median"] %>% comma(.1)` years for those in the Hall. 
